...
export const useChatStore = defineStore(
  ...
  () => {
    ...
    const conversationsById =
      ref<Record<string, Conversation>>({});

    const messagesById =
      ref<Record<string, Message>>({});
    ...
    const conversations = computed<Conversation[]>(
      () => Object.values(conversationsById.value),
    );
    ...
    const activeMessages = computed<Message[]>(() => {
      ...
      const ids =
        conversationMessageIds.value[
          conversationId
        ] ?? [];

      return ids.reduce<Message[]>(
        (result, id) => {
    ...
    async function initialize(): Promise<void> {
      ...
      try {
        const [loadedUsers, conversationList] =
          await Promise.all([
        ...
        conversationList.forEach((conversation) => {
          conversationsById.value[conversation.id] =
            conversation;
        });

        const firstConversation =
          conversationList.at(0);
    ...
    function reconcileMessage(
      ...
      const optimisticMessage =
        messagesById.value[optimisticId];
